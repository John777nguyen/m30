...
- Script tải trang, parse tin nhắn mới nhất bằng regex (không cần cài thư viện ngoài),
  so sánh với lần trước.
- Nếu có tin mới -> gửi qua Telegram Bot API tới chat cá nhân của bạn.

Yêu cầu:
    pip install requests   (chỉ cần đúng 1 thư viện ngoài)
"""
...
# ====== CẤU HÌNH ======
CHANNEL_USERNAME = "SIGNAL_BBMA_M30"   # không có @ hay t.me/
...
# Mỗi block tin nhắn nằm trong div có data-post="channel/123"
...
def _strip_html(raw: str) -> str:
    """Bỏ thẻ HTML, giải mã HTML entity, trả về text thuần."""
    text = raw.replace("<br>", "\n").replace("<br/>", "\n")
...
def fetch_messages():
    ...
    messages = []
    for msg_id, raw_text in _MESSAGE_BLOCK_RE.findall(resp.text):
...
def main():
    print(f"Đang kiểm tra kênh: {CHANNEL_USERNAME} ({PREVIEW_URL})")
